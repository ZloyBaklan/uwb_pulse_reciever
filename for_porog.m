...
= S(p, n).*hadamard_matrix(:,n+1); %формирование матрицы всех каналов с ПСП
end
end
M = (sign(M)+1)/2;
%% preamble
...
= demodulation(n,((p-1)*length(hadamard_matrix)+1):p*length(hadamard_matrix)).*hadamard_matrix(n+1,:);%умножениеобщегополученногосигналанаПСП
